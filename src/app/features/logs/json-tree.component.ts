...
export class JsonTreeComponent {
  ...
  protected readonly entries = computed(() => {
    ...
    return Object.entries(v as Record<string, unknown>).map(([key, value]) => ({
      key,
      value,
      errorKey: JsonTreeComponent.ERROR_KEYS.has(key.toLowerCase()),
      ...JsonTreeComponent.classify(value),
    }));
  });
  ...
  protected readonly scalarType = computed(
    () => JsonTreeComponent.classify(this.value()).valueType,
  );
  protected readonly scalarDisplay = computed(
    () => JsonTreeComponent.classify(this.value()).display,
  );
