...
@Component({
  ...
  templateUrl: './chunk-timeline.component.html',
  styleUrl: './chunk-timeline.component.scss',
})
export class ChunkTimelineComponent {
  ...
  protected readonly rowHeight = computed(() => {
    ...
    if (n === 0) return ChunkTimelineComponent.ROW_HEIGHT_MAX;
    const fit = ChunkTimelineComponent.MAX_PLOT_HEIGHT / n;
  ...
  protected readonly spanText = computed(() =>
    ChunkTimelineComponent.formatDuration(this.spanMs()),
  );
  ...
  protected readonly svgWidth =
    ChunkTimelineComponent.LABEL_GUTTER + ChunkTimelineComponent.PLOT_WIDTH;

  /** Left edge of the plotted bars — exposed for the axis baseline. */
  protected readonly plotLeft = ChunkTimelineComponent.LABEL_GUTTER;
  protected readonly plotWidth = ChunkTimelineComponent.PLOT_WIDTH;
  ...
  protected readonly bars = computed<TimelineBar[]>(() => {
    ...
    return sorted.map((c, index) => {
      ...
      const x =
        ChunkTimelineComponent.LABEL_GUTTER +
        ((s - start) / span) * ChunkTimelineComponent.PLOT_WIDTH;
      const rawWidth = ((e - s) / span) * ChunkTimelineComponent.PLOT_WIDTH;
      const width = Math.max(rawWidth, ChunkTimelineComponent.MIN_BAR_WIDTH);

      const durationText = running
        ? `${ChunkTimelineComponent.formatDuration(e - s)} (running)`
        : ChunkTimelineComponent.formatDuration(e - s);
